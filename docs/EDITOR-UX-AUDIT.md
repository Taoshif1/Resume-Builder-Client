...
- A dedicated document outline supports section selection, current-section indication and hidden-section restoration. Contextual actions support section ordering/visibility and entry editing, movement, duplication, customization reset and removal from a document. Checkpoint H verified these through real browser interactions and persisted API state, including preservation of Master Profile experience and Project Library records after document removal.
- Checkpoint H also verified empty-source creation, return to the resume, show/hide/restore for Certifications, Languages and Custom sections. The final full run passed 19 journeys, 120 responsive combinations and 45 axe scans, with 0 overflow, 0 axe violations, 0 page errors and 0 React key warnings. Unit/API count remains 101; H changes browser tests only.
- Checkpoint I captured and inspected 36 screenshots: default, selected section, selected entry, outline, Design, Order/Add, Edit, Preview and a multipage CV at 1440x1000, 768x1024, 390x844 and 360x800. The long CV printed to 10 pages. Evidence is in ignored `.artifacts/editor-visual`; these are review captures, not a pixel-diff baseline.
- Screenshot findings fixed: contextual labels clipped by fixed toolbar heights, mobile Export PDF truncation, mobile outline stacking beneath contextual actions, and paper focus shifting preview controls horizontally. Contextual actions now wrap, mobile action targets are at least 44px high, and horizontal paper scrolling stays inside the canvas. At 100% zoom, narrow screens still intentionally require horizontal paper panning.
- Final G-H-I verification: `npm run test:e2e` exit 0, 20 recorded journeys (19 functional plus the visual capture matrix), 120 responsive combinations, 45 axe scans, 0 violations/overflow/page errors/React key warnings. Lint, 101/101 unit/API tests, build and production dependency audit all passed after the CSS fixes.
