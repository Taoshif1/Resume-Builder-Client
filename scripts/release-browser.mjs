...
import {captureEditorVisuals} from './editor-visual.mjs';
...
catch(error){console.warn('System Chrome unavailable; using Playwright Chromium.');browser=await chromium.launch({headless:true});}
const context=await browser.newContext({viewport:{width:1440,height:1000},acceptDownloads:true});
const page=await context.newPage();page.setDefaultTimeout(15000);
const report={journeys:[],responsive:[],accessibility:[],errors:[],reactWarnings:[],print:[]};
...
page.on('console',message=>{if(['warning','error'].includes(message.type())&&/same key|unique.*key|React.*warning/i.test(message.text()))report.reactWarnings.push(message.text());});
const record=name=>{report.journeys.push(name);console.log('PASS:',name)};
async function ready(path){await page.goto(base+path);const protectedRoute=path==='/dashboard'||path.startsWith('/dashboard/')||path.startsWith('/resume/')||path==='/admin';await page.locator(protectedRoute?'.pcv-shell':'main h1').first().waitFor();await page.evaluate(()=>document.fonts.ready);}
...
try{
 ...
 await page.getByRole('navigation',{name:'Editor sections'}).getByRole('button',{name:'Design',exact:true}).click();await page.getByRole('combobox',{name:/^Paper size/}).selectOption('LEGAL');await page.getByRole('combobox',{name:/^Font/}).selectOption('serif');await page.getByLabel('Text size in points',{exact:true}).fill('12');await save();await page.reload();await page.getByRole('navigation',{name:'Editor sections'}).getByRole('button',{name:'Design',exact:true}).click();assert.equal(await page.getByRole('combobox',{name:/^Paper size/}).inputValue(),'LEGAL');assert.equal(await page.getByRole('combobox',{name:/^Font/}).inputValue(),'serif');record('Document Studio paper/font/text size save/reload');
 ...
 await save();const downloadPromise=page.waitForEvent('download');await page.getByRole('button',{name:'Export PDF',exact:true}).click();const download=await downloadPromise;await download.saveAs(`${out}/server-export.pdf`);assert.equal(readFileSync(`${out}/server-export.pdf`).subarray(0,5).toString(),'%PDF-');record('Authenticated browser PDF download signature');
 ...
 await captureEditorVisuals({page,api,ready,record,editor});
 ...
 for(const [width,height]of [[360,800],[390,844],[768,1024],[1024,768],[1280,800],[1366,900],[1440,1000],[1920,1080]]){
  ...
  for(const route of routes){await ready(route);if(route===editor&&width<=900)await page.getByLabel('Editor view').getByRole('button',{name:'Preview',exact:true}).click();
 ...
 await page.emulateMedia({reducedMotion:'reduce'});await ready(editor);await page.getByLabel('Editor view').getByRole('button',{name:'Preview',exact:true}).click();assert.ok(parseFloat(await page.locator('.pcv-paper').evaluate(e=>getComputedStyle(e).transitionDuration))<=0.001);record('Reduced motion preview');
 ...
 assert.equal(report.responsive.filter(r=>!r.pass).length,0,'No responsive overflow');
 assert.equal(report.accessibility.filter(r=>r.violations.length).length,0,'No axe violations');
 assert.deepEqual(report.errors,[],'No uncaught browser errors');
 assert.deepEqual(report.reactWarnings,[],'No React key warnings');
